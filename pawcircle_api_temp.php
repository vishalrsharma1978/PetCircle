...
switch ($action) {
    ...
    case 'get_family_data':
        handleGetFamilyData($inputData);
        break;
    case 'save_family_member':
        handleSaveFamilyMember($inputData);
        break;
    case 'delete_family_member':
        handleDeleteFamilyMember($inputData);
        break;
...
function handleGetFamilyData($data)
{
...
function handleSaveFamilyMember($data)
{
...
function handleDeleteFamilyMember($data)
{
    $userId = cleanNullableText($data['user_id'] ?? '', 80);
...
function handleSaveBirthDetails($data)
{
    ...
    $memberId = cleanNullableText($data['member_id'] ?? 'self', 80) ?: 'self';
    $birthRow = [
        'date_of_birth' => cleanDateValue($data['dob'] ?? null),
        'birth_time' => cleanTimeValue($data['birthTime'] ?? ($data['time'] ?? null)),
        'birth_city' => cleanNullableText($data['birthCity'] ?? ($data['city'] ?? null), 180),
        'gender' => cleanNullableText($data['gender'] ?? null, 40),
    ];

    if ($memberId === 'self') {
        $row = array_merge(['user_id' => $userId], $birthRow);
        $res = supabaseRequest(
            'POST',
            '/rest/v1/user_horoscope_profiles',
            ['on_conflict' => 'user_id'],
            $row,
        ...
        $res = supabaseRequest(
            'PATCH',
            '/rest/v1/family_members',
            ['id' => 'eq.' . $memberId, 'owner_user_id' => 'eq.' . $userId],
            $birthRow,
            ['Prefer: return=representation']
        );
    }
    ...
    if (($res['code'] ?? 500) >= 400) {
        jsonError("Failed to save birth details.", 500, ["supabase_response" => $res['data']]);
        return;
    }
...
function handleJoinMandal($data)
{
