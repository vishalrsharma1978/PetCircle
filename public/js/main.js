...
// Patch lucide.createIcons to debounce global DOM scans and fix massive UI lag
if (typeof window !== 'undefined' && !window.__lucidePatched) {
  window.__lucidePatched = true;
  const interval = setInterval(() => {
    if (typeof lucide !== 'undefined' && lucide.createIcons && !lucide.__patched) {
      lucide.__patched = true;
      const originalCreateIcons = lucide.createIcons;
      let lucideTimeout;
      lucide.createIcons = function (params) {
        if (params && params.root) {
          return originalCreateIcons.call(this, params);
        }
        clearTimeout(lucideTimeout);
        lucideTimeout = setTimeout(() => {
          originalCreateIcons.call(this, params);
        }, 50);
      };
...
async function loadSupabaseAuthClient() {
  ...
  if (!pawcircleAuthConfigPromise) {
    ...
    pawcircleAuthConfigPromise = fetch("api/index.php", {
...
async function toggleNotificationsPanel() {
  ...
  if (willOpen && typeof closeProfileMenu === 'function') {
    closeProfileMenu();
  }
...
function getUnreadNotificationsPopupStorageKey() {
  ...
  return `pawcircle_unread_notifications_popup_dismissed_time_${userKey}`;
}

function getDismissedUnreadNotificationsTime() {
  try {
    // FIX: Treat this strictly as a boolean flag
    return sessionStorage.getItem(getUnreadNotificationsPopupStorageKey()) === "true";
  } catch (e) {
    return false;
  }
}

function setDismissedUnreadNotificationsTime() {
  try {
    sessionStorage.setItem(getUnreadNotificationsPopupStorageKey(), "true");
  } catch (e) { }
}
...
function getLatestUnreadTime() {
  const items = [...(globalNotifications || [])];

  // We don't want the synthetic complete_profile notification to trigger the popup repeatedly
  // since its created_at is always "now".
  const realUnread = items.filter((n) => !n.is_read && n.type !== "complete_profile");

  let latestTime = 0;
  for (const n of realUnread) {
...
function updateUnreadNotificationsPopup(unreadCount) {
  ...
  // FIX: If it was dismissed once this session, never show it again
  if (getDismissedUnreadNotificationsTime()) {
    clearTimeout(unreadPopupAutoHideTimer);
    popup.classList.add("hidden");
    return;
  }
...
function dismissUnreadNotificationsPopup() {
  setDismissedUnreadNotificationsTime();
...
document.addEventListener("click", (event) => {
  ...
  if (path.includes(panel) || path.some(el => el.id === "notifications-toggle-btn")) return;
    ...
    overlay.innerHTML = `
          ...
          <div class="w-full max-w-sm bg-white dark:bg-gray-900 rounded-2xl shadow-2xl border border-gray-100 dark:border-gray-800 p-6 transform scale-95 transition-transform duration-200">
            ...
            <div class="mt-6 flex justify-end gap-2">
              ...
              ${hideCancel ? "" : `<button data-act="cancel" class="px-4 py-2 rounded-xl text-base font-bold text-gray-700 dark:text-gray-200 bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors">${escapeHtml(cancelText)}</button>`}

              <button data-act="confirm" class="px-4 py-2 rounded-xl text-base font-bold text-white ${accent.btn} transition-colors">${escapeHtml(confirmText)}</button>
...
function toggleEventInterest(eventId, interested) {
  ...
  if (interested) {
    ...
    btnContainer.innerHTML = `<button class="w-full bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400 py-2 rounded-lg text-base font-bold transition-colors flex items-center justify-center gap-2 shadow-sm"><i data-lucide="check-circle-2" class="w-4 h-4"></i> You're Interested</button>`;
